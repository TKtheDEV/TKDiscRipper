...
class OtherDiscRipper(BaseRipper):
    ...
    def rip(self):
        ...
        if code != 0:
            update_job(self.job_id, progress=100, operation="failed")
            yield f"❌ dd failed"
            return
    ...
    def _compress_iso(self, iso_path: str):
        ...
        try:
            if ext == "bz2":
                update_job(self.job_id, log="▶️ Starting compression using bzip2...", operation="Compressing...")
                compress_bz2(iso_path, final_path)
            elif ext == "zst":
                update_job(self.job_id, log="▶️ Starting compression using bzip2...", operation="Compressing...")
                compress_zst(iso_path, final_path)
            else:
                update_job(self.job_id, log=f"⚠️ Unknown format '{ext}', skipping compression...")
